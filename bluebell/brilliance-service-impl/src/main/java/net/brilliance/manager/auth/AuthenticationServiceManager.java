...
public class AuthenticationServiceManager implements Serializable{
	...
	protected void setupPermissionsByExample(){
		...
		Optional<AccessRight> viewAccessRight = accessRightManager.getByName(DefaultAccessRightAction.View.getAction());
		Optional<AccessRight> createAccessRight = accessRightManager.getByName(DefaultAccessRightAction.Create.getAction());
		Optional<AccessRight> modifyAccessRight = accessRightManager.getByName(DefaultAccessRightAction.Modify.getAction());
		Optional<AccessRight> deleteAccessRight = accessRightManager.getByName(DefaultAccessRightAction.Delete.getAction());

		List<Module> modules = moduleManager.getAll();

		Optional<Authority> adminRole = authorityManager.getByName(AuthorityGroup.RoleAdmin.getCode());
		Optional<Authority> clientRole = authorityManager.getByName(AuthorityGroup.RoleClient.getCode());
		for (Module module :modules){
			this.permissionManager.save(Permission.getInstance(adminRole.get(), module, viewAccessRight.get()));
			this.permissionManager.save(Permission.getInstance(adminRole.get(), module, createAccessRight.get()));
			this.permissionManager.save(Permission.getInstance(adminRole.get(), module, modifyAccessRight.get()));
			this.permissionManager.save(Permission.getInstance(adminRole.get(), module, deleteAccessRight.get()));
		}

		for (Module module :modules){
			this.permissionManager.save(Permission.getInstance(clientRole.get(), module, viewAccessRight.get()));
			this.permissionManager.save(Permission.getInstance(clientRole.get(), module, createAccessRight.get()));
			this.permissionManager.save(Permission.getInstance(clientRole.get(), module, modifyAccessRight.get()));
			this.permissionManager.save(Permission.getInstance(clientRole.get(), module, deleteAccessRight.get()));
		}
